...
export function Sparkline({ data }) {
  if (!data || data.length < 2) {
    return <div className="h-10 flex items-center text-[11px] text-gray-300 font-medium">Not enough data yet</div>;
  }
  return (
...
export function SkillRadar({ axes, learnerName, learnerPhotoUrl }) {
  ...
  return (
    <div className="relative h-64 -mx-2">
      <ResponsiveContainer width="100%" height="100%">
        <RadarChart data={axes} outerRadius="50%" margin={{ top: 8, right: 42, bottom: 8, left: 42 }}>
          <PolarGrid stroke="#eef0f4" />
          <PolarAngleAxis dataKey="label" tick={{ fontSize: 10, fill: '#6b7280', fontWeight: 600 }} />
