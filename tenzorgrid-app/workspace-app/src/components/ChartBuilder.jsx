...
function Choice({ label, children }) {
  return (
    <div>
      <p className="text-[12px] font-extrabold uppercase tracking-wide text-slate-500 mb-1.5">{label}</p>
      {children}
    </div>
  );
}
  ...
  return (
    <div className="flex flex-col min-w-0">
      <div className="flex items-center justify-between gap-3 px-3 py-2 border-b border-slate-200 bg-slate-50/60">
        <span className="text-[12px] font-bold text-slate-500 uppercase tracking-wider">Chart Builder</span>
        <button
          onClick={send}
