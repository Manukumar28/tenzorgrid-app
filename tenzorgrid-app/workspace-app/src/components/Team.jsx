  ...
  return (
    <div className="space-y-5">
      <BentoCard index={0} hover={false}>
        <h3 className="text-base font-bold mb-3.5">Your people</h3>
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
          {state.roster.map((p) => (
            <button
              key={p.archetype}
              onClick={() => setSelected(p.archetype)}
              className={`text-left border rounded-lg p-4 ${p.archetype === selected ? 'border-indigo-300 bg-indigo-50' : 'border-gray-100 hover:border-gray-200'}`}
            >
              <Avatar name={p.name} avatarUrl={p.avatarUrl} size={44} className="mb-3" />
        ...
        <BentoCard index={1} hover={false}>
          {isEmailOnly ? (
            <>
              ...
              <p className="text-sm text-gray-500">External contacts like {person.name} are reached through Emails, not Team Chat.</p>
            </>
          ) : (
            <>
              <div className="flex items-center gap-3.5 mb-3.5">
                <Avatar name={person.name} avatarUrl={person.avatarUrl} size={48} />
                <h3 className="text-base font-bold">{person.name} · {person.title}</h3>
              </div>
              <div className="space-y-3 max-h-80 overflow-y-auto pr-1 mb-3.5">
                {thread.length ? thread.map((m) => (
                  <div key={m.id} className={`rounded-lg px-4 py-3 text-sm ${m.sender_archetype === 'learner' ? 'bg-indigo-50 ml-6' : 'bg-gray-50'}`}>
                    <div className="text-xs font-bold text-gray-500 mb-1">{SENDER_LABEL[m.sender_archetype] || m.sender_name}</div>
                    {m.body}
                  </div>
